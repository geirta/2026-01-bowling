...
function Home() {
  ...
  return (
    <div className='container'>
          ...
          <tbody>
            {game?.players && game.players.length > 0 
              ? (game.players.map(player => {
                ...
                return (
                  <tr key={player.name}
                    className={player.name === game?.currentPlayer?.name ? "activePlayer" : ""}
                  >
                      ...
                      return (
                        <td key={i}>
                          <div className="frame-throws">
                            <span className="throw">{frame?.[0] === 10 
                              ? "X" 
                              : frame?.[0] ?? ""}
                            </span>
                            <span className="throw">
                              {frame?.[0] !== undefined && frame?.[1] !== undefined && frame[0] + frame[1] === 10 
                                ?  "/"
                                : frame?.[1] ?? ""
                              }
                            </span>

                            {i === 9 && frame?.[2] !== undefined && (
                              <span className="throw">{frame[2]}</span>
                            )}
                          </div>
                          <div className="frame-score">
                            {frameScore != null ? runningTotal : ""}
                          </div>
                        </td>
                        )
                    })}
      ...
      <div className="turns">
        <h1 className="playerName">{game.currentPlayer.name}</h1>
        <div className="throwButton">
          <button className="btnStyle" onClick={throwBall}>Throw ball</button>
        </div>
        <div className="pinsUp">
          <h1>Pins up: {game.pins}</h1>
        </div>
      </div>
      )}
